 ...
 *
 * Optional attributes:
 *   limit="4"           Max products to show (default 4)
 *   orderby="date"      date | title | rand | menu_order (default date)
 *   order="DESC"        ASC or DESC (default DESC)
 *   source="featured"   featured | recent | manual
 *   ids="12,34,56"      Comma-separated product IDs when source="manual"
 */
function vibecano_featured_products_shortcode( $atts ) {
	if ( ! class_exists( 'WooCommerce' ) ) {
		return '<!-- WooCommerce is required for Vibecano Featured Products -->';
	}
	...
	if ( $atts['source'] === 'manual' && ! empty( $atts['ids'] ) ) {
	...
	if ( empty( $products ) ) {
		return '<!-- No featured products found. Mark products as Featured in WooCommerce → Products → Edit product → Catalog visibility. -->';
	}
...
/**
 * Allow shortcodes inside Elementor HTML widgets.
 */
add_filter(
